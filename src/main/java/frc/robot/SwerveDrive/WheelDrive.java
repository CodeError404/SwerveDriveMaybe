...
public double encoderValue;
	...
	public WheelDrive (int angleMotor, int speedMotor, int analogIn, double P, double I, double D) {
		...
		//create our "wheels"
		this.angleMotor = new TalonSRX(angleMotor, MotorType.kBrushless);
		this.speedMotor = new TalonSRX(speedMotor, MotorType.kBrushless);

		
		this.azimuthEncoder = new AnalogInput(analogIn);

		anglePID = new PIDController(P, I, D, source, output);
		anglePID = new PIDController(Kp, Ki, Kd, source, output, period);
